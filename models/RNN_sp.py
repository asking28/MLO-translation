...
class RNN_MODEL(nn.Module):
    def __init__(self,criterion,src_tokenizer,trg_tokenizer,max_length=128,MODEL=None):
        ...
        self.EOS_token = src_tokenizer.eos_id()
        self.SOS_token = src_tokenizer.bos_id()
...
#         trg = trg.permute(1,0)
        loss_vec = self._criterion(t.output.permute(1,2,0),trg)
        return loss_vec
...
#                 decoder_input = torch.cuda.LongTensor([self.tokenizer.bos_token_id for _ in range(batch_size)])
    ...
    #                 print(topi)
                ...
                pad_token_id = self.tokenizer.unk_id()
                eos_token_ids = [self.tokenizer.eos_id()]

                decoder_hidden = hidden
                decoded_batch = torch.zeros((batch_size, self.MAX_LENGTH),dtype=torch.int32).to(device)
                decoder_input = torch.cuda.LongTensor([self.tokenizer.bos_id() for _ in range(batch_size)])
